...
public class Psse30Export
{
	...
	static class BranchBuses
	{
		final long key;
		BranchBuses(int fbus, int tbus)
		{
			int b2, b1;
			if (fbus < tbus)
			{
		...
		{
			return key == ((BranchBuses)obj).key;
		}
		
	}
	int getSingleBus(Bus b) throws PsseModelException
	{
		return _sbus.findGrpNdx(b)+1;
	}

	protected enum Type {Gen,Load,Line};
	Map<Type,Map<Integer,Integer>> _t1id = new EnumMap<>(Type.class);
	Map<Type,Map<BranchBuses,Integer>> _t2id = new EnumMap<>(Type.class);
	
	int getNextID(Type type, int bus)
	{
		Map<Integer,Integer> m = _t1id.get(type);
		if (m == null)
		{
			m = new HashMap<>();
			_t1id.put(type, m);
		}
		Integer rv = m.get(bus);
		if (rv == null)
		{
			rv = Integer.valueOf(0);
		}
		m.put(bus, ++rv);
		return rv;
	}
	int getNextID(Type type, int fbus, int tbus)
	{
		Map<BranchBuses,Integer> m = _t2id.get(type);
		if (m == null)
		{
			m = new HashMap<>();
			_t2id.put(type, m);
		}
		BranchBuses key = new BranchBuses(fbus, tbus);
		Integer rv = m.get(key);
		if (rv == null)
		{
			rv = Integer.valueOf(0);
		}
	...
	{
		// TODO: handle ownership
		int fb = getSingleBus(l.getFromBus());
		int tb = getSingleBus(l.getToBus());
		if (l.getMeteredEnd() == LineMeterEnd.To) tb *= -1;
		_o.format(
				"%d, %d, '%2s', %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %d, %f\n",
				fb, tb, getNextID(Type.Line, fb, tb), l.getR(), l.getX(), l.getB(), l.getRATEA(),
	...
	public void expTransformer(Transformer t) throws PsseModelException
	{
		...
		int fb = getSingleBus(t.getFromBus());
		int tb = getSingleBus(t.getToBus());
	...
	{
		// TODO: implement ownership
		int fb = getSingleBus(p.getFromBus());
		int tb = getSingleBus(p.getToBus());
	...
	{
		Bus rb = _m.getBuses().get(s.getSWREM());
	...
	{
		_sbus = new BusGroup2TDevList(_m){
